...
def generate_subkeys(key_bits):
    ...
    for shift in shift_schedule:
        ...
        combined = C + D
         # Apply PC1 permutation to get 48 bits
...
def des_encrypt(plaintext, key):
    ...
    for key in subkeys:
        new_left = right.copy()
        round_output = round_f(right, key)
        new_right = [l ^ f for l, f in zip(left, round_output)]
...
def des_decrypt(ciphertext, key):
    ...
    key_bits = bytes_to_bits(key)
    subkeys = generate_subkeys(key_bits)  # Reverse subkeys
    # Initial permutation
    ...
    for key in reversed(subkeys):
        new_left = right.copy()
        feistel_output = round_f(right, key)
